package developer.anikesh.atc.views
...
class LoginButton @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.LoginButton, 0, 0)
            mColorPrimary = ta.getInt(R.styleable.LoginButton_lb_btnColor, Color.BLUE)
            mDisabledBgColor = ta.getColor(R.styleable.LoginButton_lb_btnDisabledColor, Color.LTGRAY)
            mDisabledTextColor = ta.getColor(R.styleable.LoginButton_lb_disabledTextColor, Color.DKGRAY)
            val text = ta.getString(R.styleable.LoginButton_lb_btnText)
            mText = text ?: ""
            mTextColor = ta.getColor(R.styleable.LoginButton_lb_textColor, Color.WHITE)
            resetAfterFailed = ta.getBoolean(R.styleable.LoginButton_lb_resetAfterFailed, true)
            mRippleColor = ta.getColor(R.styleable.LoginButton_lb_btnRippleColor, Color.BLACK)
            mRippleAlpha = ta.getFloat(R.styleable.LoginButton_lb_btnRippleAlpha, 0.3f)
            mButtonCorner = ta.getFloat(R.styleable.LoginButton_lb_cornerRadius, 2 * mDensity)
            ta.recycle()
        }
